...
public class FileManager {
	...
	/**
	 * 파일 정보 조회 (/** + 엔터) 매개변수 입력 후파라미터 자동생성
	 * @param filePath
	 */
	...
	private static void deleteDirectory(String path) {
		...
		//		: 폴더 안에 폴더가 있으면?? -> 재귀호출(Factorial)로 삭제
		// 1. 하위 경로의 파일들을 삭제
		// 2. 해당 폴더 삭제
		File file = new File(path);
	...
	public static void main(String[] args) {
		int menuNo;
		// FileManager fm = new FileManager(); 를 만들던지 밑에 메뉴에 static붙이던지
		Scanner sc = new Scanner(System.in);
		do {
			// TODO: 정의한 메소드 기능을 활용하여 파일 관리 프로그램이 실행되도록 완성
			// 메뉴판 출력
			menu();
